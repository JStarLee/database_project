...
<!--查询表单-->
    <el-form :inline="true" class="demo-form-inline" ref="sectionQuery" :model="sectionQuery" :rules="rules">
      <el-form-item label="授课人：" prop='tid'>
        <el-input v-model="sectionQuery.tid" placeholder="教师号" />
      </el-form-item>
    ...
    <el-table :data="list" border fit highlight-current-row>
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
        <!-- scope.row.sec_id -->
            <el-button type="primary" size="mini" icon="el-icon-circle-plus-outline" @click="select(scope.row.sec_id,sectionQuery.tid)">授课</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
    data(){
        const isExist = (rule, value, callback)=>{
          ...
          for (var i=0;i<this.allTeacher.length;i++){
            if(value == this.allTeacher[i]['tid']){
              callback()
            }
          }
          callback(new Error('输入的教师号不存在'))
        }
        return{
            teach: {
                Teacher:'',
            ...
            rules: {
              tid:[
                {required:true,message:'须指定授课人',trigger:'blur'},
    ...
    created(){
            this.init()     
            this.$alert('请指定授课人进行查询以获取可选课程', '提示', {
    ...
    methods: {
        ...
        getAllTeacher(){
            teacherApi.getAllTeacher()
                .then(response => {
                    this.allTeacher = response.data.allTeacher;
                })
        },
        saveOrUpdate(){
        ...
        getList(page = 1) {
          if(this.sectionQuery['tid']=='' || this.sectionQuery['tid']==undefined ||this.sectionQuery['tid'] ==null){
                      // 清空数据
            this.list = []
            this.total = 0;
            return;
          ...
          this.page = page;
          section
              .getTeachSectionListPage(this.page, this.size, this.sectionQuery)
              .then(response => {
              //请求成功
              //response接口返回的数据
        ...
        select(sec_id,tid){
          this.teach.section=sec_id
          this.teach.Teacher=tid
          this.teach.status='ing'
          this.saveOrUpdate()
        }
    }
}
